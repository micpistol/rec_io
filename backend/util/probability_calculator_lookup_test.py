#!/usr/bin/env python3
"""
TEST PROBABILITY CALCULATOR LOOKUP

A probability calculator that uses the existing test probability table
...
class ProbabilityCalculatorLookupTest:
    """
    Probability calculator that uses the test master lookup table.
    ...
    def __init__(self, symbol: str = "btc", db_config: Optional[Dict] = None):
        ...
        self.table_name = f"test_master_probability_lookup_{self.symbol}"
        ...
        logger.info(f"✅ Initialized test lookup calculator for {self.symbol.upper()}")
    ...
    def calculate_strike_probabilities(self, current_price: float, ttc_seconds: float, 
        ...
        """
        Calculate strike probabilities using the test lookup table.
        
        Args:
            current_price: Current BTC price
            ttc_seconds: Time to close in seconds (must be 1800-1830 for test table)
        ...
        """
        try:
            # Validate TTC range for test table (1800-1830 seconds)
            if ttc_seconds < 1800 or ttc_seconds > 1830:
                raise ValueError(f"TTC {ttc_seconds}s is outside test table range (1800-1830s)")
            ...
            for strike in strikes:
                ...
                # Ensure buffer is within range (0-100 for test table)
                buffer_points = max(0, min(100, int(round(buffer_points))))
    ...
    def _find_closest_momentum_bucket(self, momentum_score: float) -> int:
        ...
        Returns:
            Momentum bucket (-5 to +5 for test table)
        """
        # Clamp momentum to valid range for test table
    ...
    def _get_probability_from_lookup(self, ttc: int, buffer: int, momentum: int) -> Tuple[float, float]:
        ...
        Args:
            ttc: Time to close in seconds (1800-1830)
            buffer: Buffer points (0-100)
            momentum: Momentum bucket (-5 to +5)
        ...
        try:
            ...
            if result:
                ...
                prob_positive = float(result[0]) if result[0] is not None else 0.0
                prob_negative = float(result[1]) if result[1] is not None else 0.0
    ...
    def test_lookup_performance(self, num_tests: int = 1000) -> Dict:
        ...
        try:
            # Generate random test parameters within test table range
            test_cases = []
            for _ in range(num_tests):
                ttc = random.randint(1800, 1830)  # 1800-1830 seconds
                current_price = 120000  # Base BTC price
                buffer = random.randint(0, 100)  # 0-100 points
            ...
            logger.info(f"📊 Test lookup performance:")
...
def get_probability_calculator_lookup_test(symbol: str = "btc") -> ProbabilityCalculatorLookupTest:
    """
    Factory function to create a test lookup probability calculator.
    
    Args:
        symbol: Trading symbol (default: "btc")
        
    Returns:
        ProbabilityCalculatorLookupTest instance
    """
    return ProbabilityCalculatorLookupTest(symbol=symbol)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Test probability lookup calculator')
    ...
    calculator = ProbabilityCalculatorLookupTest("btc")
    
    if args.test_performance:
        calculator.test_lookup_performance()
    
    elif args.compare_with_live:
        # Create test cases within test table range
        test_cases = [
            {'current_price': 120000, 'ttc_seconds': 1800, 'strikes': [120500], 'momentum_score': 0.0},
            {'current_price': 120000, 'ttc_seconds': 1815, 'strikes': [119500], 'momentum_score': 2.0},
            {'current_price': 120000, 'ttc_seconds': 1830, 'strikes': [121000], 'momentum_score': -3.0},
        ]
